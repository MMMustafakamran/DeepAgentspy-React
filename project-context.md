...
QA on the CopilotKit React DeepAgents docs
(<https://docs.copilotkit.ai/deepagents>). The job is **finding bugs and
ambiguity in those doc pages**. The deliverable is `DOCUMENTED_REPORT.md`, built by `ci/build-report.mjs`, plus one
...
| `markdown-rendering` | No path to it from the docs sidebar, so not under test yet. |
