...
export async function waitForAgentResponseCompletion(
  ...
  messageSelector: string = DEFAULT_ASSISTANT_MESSAGE_SELECTOR,
  /**
   * How long to wait for a reply to *start*. 30s suits a plain chat turn and is
   * deliberately the default, because a page that never answers is the failure
   * this suite exists to catch and a generous ceiling everywhere would blunt it.
   *
   * Raise it for a graph that is legitimately slow rather than broken: the
   * manual predictive variant emits four steps a second apart before it says
   * anything, and has been measured at 50s end to end. Waiting 30s for that and
   * calling it silence reports a working feature as dead.
   */
