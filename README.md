...
| | |
|---|---|
| **Doc-sync date** | `syncedAt` in `doc-snapshot/manifest.json` is the only one, 2026-09-21 at the last sync |
...
| **CopilotKit (npm)** | declared `^1.69.0` · lockfile 1.69.0 · installed 1.71.0 (`frontend/VERSIONS.md`) for `react-core`, `runtime` and `a2ui-renderer` alike |
...
**`/intelligence/learned-skills`** — ❌ **Broken as published.** New upstream 2026-09-15, restructured 2026-09-21. Automatic delivery of a Learning container's published Skills through a framework-native adapter. **Try:** just open it, then `List the skills you can load, then load the refund-policy skill and follow it.` **What happens:** the agent answers from its own instructions with no tool call, because no adapter is mounted and none can be. Every Python package on the page's table is 404 on PyPI; the BuiltInAgent row added on 2026-09-21 uses the package this repo does install and does not typecheck against it. Both BuiltInAgent snippets are in the repo verbatim (`built-in-agent-classic.ts`, `built-in-agent-factory.ts`), imported by nothing, with their compiler errors acknowledged in place. See §9 #26.
...
| [intelligence/learned-skills](https://docs.copilotkit.ai/deepagents/intelligence/learned-skills) | `/intelligence/learned-skills` | `sample_agent` | ❌ Broken | New 2026-09-15. Every Python adapter 404s on PyPI; the BuiltInAgent row added 2026-09-21 needs runtime 1.73.0, which the page does not name — §9 #26 |
...
Three pages identical under every framework prefix. Verified against `@copilotkit/react-core` and `@copilotkit/runtime` **1.71.0** (what CI resolves; the lockfile pins 1.69.0), a live `langgraph dev`, and an OpenAI key. This harness has **no** `CPK_INTELLIGENCE_API_KEY`, locally or in CI.
...
**26. Automatic learned skill delivery: the one adapter whose package exists does not compile.**
...
The 2026-09-21 sync replaced Learning's one-line pointer to learned-skills with a three-step **Set up automatic skill delivery** section. Step 2 prints an **Agent server environment** block (`CPK_INTELLIGENCE_API_KEY`, `CPK_INTELLIGENCE_LEARNING_CONTAINER_ID=expense-review`) and sends you to the framework adapter examples for "LangGraph Python, LangGraph TypeScript, Mastra, Google ADK, or Microsoft Agent Framework", a list that omits BuiltInAgent, the only row of that table with a CopilotKit package. For a Deep Agents backend the named adapter is the Python one, which is 404 on PyPI, so the environment block configures nothing and step 3's "Verify delivery in a new invocation" has nothing to verify. The step's own links resolve: `#native-setup`, `#configure-one-container` and `#deployment-requirements` all exist on the target page. Everything else the section adds is dashboard-only: the **Skill delivery** toggle, **Set up skill delivery**, **Edit schedule**, **Next scheduled run**, **Start manual run now**, **Analysis results**, the 15-eligible-Thread threshold and the 02:00 UTC default. All of it needs a provisioned project and a login this harness does not have. Two of the three new troubleshooting rows are about that surface.
...
**36. Two pages now send you to Inspector tabs that do not exist**
...
**37. Message history: the recommended recipe is a prop no release has, and the runtime recipe needs an endpoint Deep Agents does not serve**
...
- [Automatic learned skill delivery](https://docs.copilotkit.ai/deepagents/intelligence/learned-skills)
