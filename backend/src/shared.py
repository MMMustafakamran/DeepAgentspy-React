...
(`openai:gpt-5.6-luna` on Quickstart and Tool Rendering, `gpt-5.6-luna` on Dynamic Schema
...
reads `OPENAI_MODEL` and defaults to the Quickstart's `gpt-5.6-luna`. The per-page
...
#region model
# Bare id, e.g. "gpt-5.6-luna". `create_deep_agent(model=...)` wants the
