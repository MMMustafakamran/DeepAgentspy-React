...
and an [AG-UI middleware](/deepagents/agentic-protocols/ag-ui-middleware) that applies it.
...
[`selfManagedAgents` or the local development option](/deepagents/backend/self-managed-agents).
...
- [AG-UI Middleware](/deepagents/agentic-protocols/ag-ui-middleware): the middleware
  lifecycle and the `Middleware` base class.
- [Self-managed agents](/deepagents/backend/self-managed-agents): passing your own agent
  instances to the frontend provider.
- [AgentRunner and persistence](/deepagents/backend/agent-runner): where the runtime keeps
  thread history.
