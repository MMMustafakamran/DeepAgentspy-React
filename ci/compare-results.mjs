 ...
 *
 * The baseline is `autorecorder/expected-results.json`: one entry per page id
 * holding the verdict a human signed off on (pass, or fail with a named error
 * class) and why. This script reads every results file a run produced --
 * `RECORD_RESULTS*.json` from each shard, falling back to `RUN_REPORT*.json`
 * for older packages -- reduces each page to a signature, and classifies it:
 *
...
/** Union every shard's page records. Raw results win over flattened ones for the same id. */
function collectRecords(dir) {
  ...
  for (const f of raw) {
    ...
    for (const r of run.results ?? []) byId.set(r.id, { ...r, source: f });
  }
  for (const f of flat) {
    ...
    for (const v of rep.videos ?? []) {
      if (!v.id || byId.has(v.id)) continue; // no id: DeepAgents-style rows, cannot be compared
      byId.set(v.id, { ...v, source: f });
    }
  }
